...
test('ja-no-abusage', async () => {
  const { linter } = await setupLinter()
  const result = await linter.fixText("`logger` と `crashReporter` の middleware を適応した `createStore` 関数を作る", "README.md");
  expect(result.output).toBe("`logger` と `crashReporter` の middleware を適用した `createStore` 関数を作る")
})
...
test('ja-space-around-link', async () => {
  const { linter } = await setupLinter()
  const result = await linter.fixText("ほげ[README](./README.md)ほげほげ", "README.md");
  expect(result.output).toBe("ほげ [README](./README.md) ほげほげ")
})

test('ja-space-between-half-and-full-width', async () => {
  const { linter } = await setupLinter()
  const result = await linter.fixText("英語English・日本語Japanese", "README.md");
...
test('prefer-tari-tari', async () => {
  const { linter } = await setupLinter()
  const result = await linter.lintText("プログラムを書いたり、文章を書いている", "README.md");
